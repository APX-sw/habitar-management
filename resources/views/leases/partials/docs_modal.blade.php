<!-- Modal Documentos -->
<div id="docs-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.6); z-index: 1000; justify-content: center; align-items: center; backdrop-filter: blur(4px);">
    <div class="card" style="width: 850px; max-width: 95%; background: white; border-radius: 15px; padding: 0; overflow: hidden; animation: modalAppear 0.3s ease-out;">
        <div style="padding: 1.5rem; background: #f8fafc; border-bottom: 1px solid #edf2f7; display: flex; justify-content: space-between; align-items: center;">
            <div>
                <h3 style="margin: 0; color: var(--primary-color);">Documentos del Contrato</h3>
                <p id="modal-lease-location" style="margin: 0.2rem 0 0; font-size: 0.8rem; color: var(--text-light);"></p>
            </div>
            <button onclick="closeDocsModal()" style="background: none; border: none; font-size: 1.5rem; cursor: pointer; color: #a0aec0;">&times;</button>
        </div>
        ...
        <div style="padding: 1.5rem 2rem;">
            ...
            <div style="display: flex; gap: 1rem; margin-bottom: 1.5rem; align-items: center;">
                <div style="position: relative; flex: 1;">
                    ...
                    <input type="text" id="docs-search" oninput="renderDocuments()" placeholder="Buscar documento por nombre..." style="width: 100%; padding: 0.7rem 1rem 0.7rem 2.5rem; border-radius: 10px; border: 1px solid #e2e8f0; font-size: 0.9rem; outline: none; transition: border-color 0.2s;" onfocus="this.style.borderColor='var(--accent-color)'" onblur="this.style.borderColor='#e2e8f0'">
                </div>
                <div style="display: flex; gap: 0.5rem;">
                    <button onclick="setSort('desc')" id="sort-desc" class="btn" style="padding: 0.6rem 1rem; font-size: 0.8rem; background: #f1f5f9; color: #475569; display: flex; align-items: center; gap: 0.4rem; font-weight: 600; border: 1px solid #e2e8f0;">
                        <span>Recientes</span>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"></polyline></svg>
                    </button>
                    <button onclick="setSort('asc')" id="sort-asc" class="btn" style="padding: 0.6rem 1rem; font-size: 0.8rem; background: #f1f5f9; color: #475569; display: flex; align-items: center; gap: 0.4rem; font-weight: 600; border: 1px solid #e2e8f0;">
                        <span>Antiguos</span>
            ...
            <!-- Upload area -->
            <div id="drop-zone" onclick="document.getElementById('file-input').click()" style="border: 2px dashed #cbd5e0; border-radius: 10px; padding: 1.2rem; text-align: center; margin-bottom: 1.5rem; background: #fdfdfd; cursor: pointer; transition: all 0.2s;">
                <input type="file" id="file-input" style="display: none;" onchange="handleFileSelect(this)">
                <div id="upload-status" style="display: none; margin-bottom: 0.5rem; font-weight: 700; color: var(--accent-color); font-size: 0.8rem;">Subiendo...</div>
                ...
                <p style="margin: 0; font-size: 0.8rem; color: #718096;">Sube contratos, recibos o imágenes.</p>
            </div>

            <div id="docs-list" style="max-height: 400px; overflow-y: auto; border: 1px solid #f1f5f9; border-radius: 12px;">
        ...
        <div style="padding: 1.2rem; background: #f8fafc; border-top: 1px solid #edf2f7; text-align: right;">
            <button onclick="closeDocsModal()" class="btn" style="background: var(--primary-color); color: white; padding: 0.7rem 2rem;">Cerrar</button>
        </div>
    </div>
</div>
...
<script>
    ...
    function openDocsModal(leaseId, location) {
        currentLeaseId = leaseId;
        document.getElementById('modal-lease-location').innerText = location;
    ...
    function updateSortButtons() {
        document.getElementById('sort-desc').classList.toggle('btn-active-sort', currentSort === 'desc');
        document.getElementById('sort-asc').classList.toggle('btn-active-sort', currentSort === 'asc');
    }

    async function fetchDocuments() {
        try {
            const response = await fetch(`/leases/${currentLeaseId}/documents`);
            allDocuments = await response.json();
            renderDocuments();
        } catch (error) {
            document.getElementById('docs-list').innerHTML = '<p style="text-align: center; padding: 2rem; color: #c53030;">Error al cargar documentos.</p>';
        }
    }
    ...
    function renderDocuments() {
        ...
        let filtered = allDocuments.filter(d => d.filename.toLowerCase().includes(query));
        ...
        if (filtered.length === 0) {
            list.innerHTML = `<div style="padding: 3rem; text-align: center; color: var(--text-light);">
                ${allDocuments.length === 0 ? 'No hay documentos aún.' : 'No se encontraron documentos.'}
            </div>`;
            return;
        }
            ...
            return `
                <div class="doc-item" style="display: flex; justify-content: space-between; align-items: center; padding: 1rem 1.5rem; border-bottom: 1px solid #f1f5f9; transition: all 0.2s;">
                    ...
                    <div style="display: flex; gap: 0.8rem; margin-left: 1.5rem;">
                        ...
                        <button onclick="deleteDoc(${doc.id})" class="btn" style="padding: 0.5rem; background: #fff5f5; color: #c53030; border: 1px solid #feb2b2;">
    ...
    async function uploadFile(file) {
        ...
        try {
            const response = await fetch('/lease-documents', {
                method: 'POST',
                body: formData
            });
