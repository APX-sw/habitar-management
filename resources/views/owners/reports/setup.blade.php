        ...
        <form action="{{ route('reports.store') }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; background: #f8fafc; padding: 1.5rem; border-radius: 12px; border: 1px solid #edf2f7; margin-bottom: 2.5rem;">
                <div>
                    ...
                    <div style="display: flex; gap: 0.5rem; width: 100%;">
                        <div style="flex: 1.5; min-width: 140px;">
                            <select name="start_month" required style="padding: 0.8rem; border-radius: 8px; border: 1px solid #cbd5e0; background: white; font-size: 0.95rem;">
                                @php
                        ...
                        <div style="flex: 1; min-width: 90px;">
                            <select name="start_year" required style="padding: 0.8rem; border-radius: 8px; border: 1px solid #cbd5e0; background: white; font-size: 0.95rem;">
                ...
                <div>
                    ...
                    <div style="display: flex; gap: 0.5rem; width: 100%;">
                        <div style="flex: 1.5; min-width: 140px;">
                            <select name="end_month" required style="padding: 0.8rem; border-radius: 8px; border: 1px solid #cbd5e0; background: white; font-size: 0.95rem;">
                                @foreach($months as $num => $name)
                        ...
                        <div style="flex: 1; min-width: 90px;">
                            <select name="end_year" required style="padding: 0.8rem; border-radius: 8px; border: 1px solid #cbd5e0; background: white; font-size: 0.95rem;">
                                @for($y = now()->year; $y >= 2023; $y--)
                                    <option value="{{ $y }}">{{ $y }}</option>
                                @endfor
                            </select>
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
                ...
                <button type="button" id="select-all-btn" class="btn" style="background: #edf2f7; color: #4a5568; font-size: 0.8rem; padding: 0.4rem 0.8rem; font-weight: 700; border-radius: 6px;">Seleccionar Todos</button>
            </div>

            <div style="border: 1px solid #edf2f7; border-radius: 12px; max-height: 400px; overflow-y: auto; background: #fff; padding: 0.5rem;">
                @foreach($owners as $owner)
                    <label style="display: flex; align-items: center; padding: 1rem; border-bottom: 1px solid #f7fafc; cursor: pointer; transition: background 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
                        <input type="checkbox" name="owner_ids[]" value="{{ $owner->id }}" class="owner-checkbox" style="width: 1.2rem; height: 1.2rem; margin-right: 1rem; cursor: pointer;" {{ $selectedOwnerId == $owner->id ? 'checked' : '' }}>
                        <div>
                            <div style="font-weight: 700; color: var(--primary-color); font-size: 1rem;">{{ $owner->name }}</div>
                            <div style="font-size: 0.75rem; color: var(--text-light); margin-top: 0.1rem;">DNI/CUIT: {{ $owner->dni_cuit }}</div>
                        </div>
                    </label>
                @endforeach
            </div>
            ...
            <div style="margin-top: 3rem; display: flex; justify-content: flex-end; gap: 1rem;">
                <a href="{{ route('reports.index') }}" class="btn" style="background: #f7fafc; color: #4a5568; padding: 1rem 2rem; font-weight: 700;">Cancelar</a>
                <button type="submit" class="btn btn-primary" style="padding: 1rem 2.5rem; font-weight: 800; font-size: 1rem; border-radius: 10px; display: flex; align-items: center; gap: 0.5rem;">
