...
<div class="card" style="max-width: 1100px; margin: 0 auto;">
    ...
    <div style="margin-bottom: 1.5rem; padding-bottom: 1.5rem; border-bottom: 1px solid #edf2f7;">
        <h2 style="margin: 0; font-size: 1.4rem; color: var(--primary-color);">Crear Nuevo Rol</h2>
        <p style="margin: 0.3rem 0 0; color: var(--text-light); font-size: 0.9rem;">Define el nombre del rol y seleccioná los permisos que tendrá acceso.</p>
    </div>
    ...
    <form action="{{ route('roles.store') }}" method="POST">
        ...
        <div style="margin-bottom: 2rem; max-width: 380px;">
            ...
            <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="ej. Asistente de Cobros" required autofocus>
        </div>
            ...
            @foreach($groupedPermissions as $key => $section)
                @php
                    ...
                    foreach($section['permissions'] as $perm) {
                        if(in_array($perm['name'], old('permissions', []))) {
                ...
                <div class="perm-section" style="border-bottom: 1px solid #e2e8f0;">
                    ...
                    <button type="button"
                        ...
                        @if($hasChecked)
                            <span style="font-size: 0.72rem; color: var(--accent-color); background: #e6fffa; padding: 0.15rem 0.5rem; border-radius: 999px; font-weight: 600;" id="badge-{{ $key }}">Activos</span>
                        @else
                            <span style="display:none; font-size: 0.72rem; color: var(--accent-color); background: #e6fffa; padding: 0.15rem 0.5rem; border-radius: 999px; font-weight: 600;" id="badge-{{ $key }}">Activos</span>
                        @endif

                        <span style="font-size: 0.75rem; color: #a0aec0;">{{ $count }} permisos</span>

                        <svg id="arrow-{{ $key }}" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color: #a0aec0; transition: transform 0.2s; flex-shrink: 0;">
                            <polyline points="6 9 12 15 18 9"></polyline>
                        </svg>
                    </button>

                    {{-- Panel de permisos (colapsado por defecto) --}}
                    <div id="panel-{{ $key }}" style="display: none; padding: 0.6rem 1rem; background: white; border-top: 1px solid #f0f4f8;">
                        ...
                        @foreach($subModules as $subName => $perms)
                            ...
                            <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 0.4rem; padding: 0.25rem 0;">
                                @foreach($perms as $perm)
                                    <label class="perm-pill perm-pill-{{ $key }}" style="display: flex; align-items: center; padding: 0.3rem 0.8rem; border-radius: 999px; cursor: pointer; font-size: 0.83rem; font-weight: 500; transition: all 0.15s; {{ in_array($perm['name'], old('permissions', [])) ? 'background: var(--accent-color); border: 2px solid var(--accent-color); color: white; font-weight: 600;' : 'background: #f1f5f9; border: 2px solid #e2e8f0; color: #4a5568;' }}">
                                        <input type="checkbox"
                                            name="permissions[]"
                                            value="{{ $perm['name'] }}"
