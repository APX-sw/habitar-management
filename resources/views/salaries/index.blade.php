...
<div style="padding: 0.5rem;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
        <div>
            <h1 style="color: var(--primary-color); margin-bottom: 0.5rem; font-size: 2rem;">Gestión de Sueldos</h1>
            <p style="color: var(--text-light); margin: 0;">Administra los sueldos, adelantos y bonos de los empleados.</p>
        </div>
        <div style="display: flex; gap: 1rem;">
            <a href="{{ route('salaries.index') }}" onclick="event.preventDefault(); document.getElementById('advanceModal').style.display='flex';" class="btn" style="background: var(--secondary-color); color: var(--text-main); font-weight: 600; padding: 0.8rem 1.5rem; display: flex; align-items: center; border-radius: 10px;">Nuevo Adelanto</a>
            <a href="{{ route('salaries.create') }}" class="btn btn-primary" style="padding: 0.8rem 1.5rem; display: flex; align-items: center; gap: 0.5rem; border-radius: 10px; font-weight: 600;">
    ...
    <div class="card" style="margin-bottom: 2rem; padding: 1rem 1.5rem; border: 1px solid var(--secondary-color); background: #f8fafc; border-radius: 12px;">
        <form id="filter-form" action="{{ route('salaries.index') }}" method="GET" style="display: flex; gap: 1rem; align-items: center;">
            <div style="flex: 1;">
            ...
            <a href="{{ route('salaries.index') }}" class="btn" style="padding: 0.7rem 1rem; border-radius: 8px; background: #edf2f7; color: #4a5568; display: flex; align-items: center; border: 1px solid #cbd5e0;" title="Limpiar Filtros">
        ...
        <div class="card" style="border-radius: 15px; border: 1px solid var(--secondary-color);">
            ...
            @if($periods->isEmpty())
                <div style="text-align: center; padding: 5rem 2rem; background: #f8fafc; border-radius: 12px; border: 2px dashed #cbd5e0;">
                    ...
                    <a href="{{ route('salaries.index') }}" class="btn" style="background: var(--primary-color); color: white; padding: 0.8rem 2rem; border-radius: 8px;">Ver Todos los Periodos</a>
                </div>
            @else
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 1.5rem;">
                    @foreach($periods as $period)
                        ...
                        @endphp
                        <a href="{{ route('salaries.show_period', ['month' => $period->month, 'year' => $period->year]) }}" style="text-decoration: none; color: inherit; display: block;">
                            <div class="period-card" style="padding: 1.5rem; border-radius: 15px; transition: all 0.3s ease; border: 1px solid #e2e8f0; background: white; position: relative; overflow: hidden; height: 100%;">
                                ...
                                <div style="font-size: 0.85rem; color: #4a5568; margin-bottom: 1rem; display: flex; align-items: center; gap: 0.4rem;">
                                    ...
                                    {{ $period->total_settlements }} sueldos procesados
                                </div>
    ...
    <div style="background: white; padding: 2rem; border-radius: 12px; max-width: 500px; width: 100%; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
        ...
        <form action="{{ route('salaries.advance') }}" method="POST">
            ...
            <div style="margin-bottom: 1rem;">
                ...
                <select name="employee_id" required style="width: 100%; padding: 0.5rem; border-radius: 6px;">
            ...
            <div style="margin-bottom: 1rem;">
                ...
                <input type="number" step="0.01" name="amount" required style="width: 100%; padding: 0.5rem; border-radius: 6px;" placeholder="Ej: 50000">
            </div>

            <div style="margin-bottom: 1rem;">
                <label>Fecha del Adelanto *</label>
                <input type="date" name="date" value="{{ date('Y-m-d') }}" required style="width: 100%; padding: 0.5rem; border-radius: 6px;">
                <small style="color: var(--text-light);">Este adelanto se descontará automáticamente en la liquidación de este mes y año.</small>
            </div>

            <div style="margin-bottom: 1rem;">
                <label>Cuenta Origen (Caja/Banco) *</label>
                <select name="account_id" required style="width: 100%; padding: 0.5rem; border-radius: 6px;">
                    <option value="">Seleccionar cuenta de la que sale el dinero...</option>
                    @foreach(\App\Models\Account::all() as $account)
                        <option value="{{ $account->id }}">{{ $account->name }} (Saldo: ${{ number_format($account->balance, 2, ',', '.') }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="text" name="description" style="width: 100%; padding: 0.5rem; border-radius: 6px;" placeholder="Ej: Adelanto por vacaciones">
            </div>

            <div style="text-align: right;">
                <button type="button" onclick="document.getElementById('advanceModal').style.display='none'" class="btn" style="background: #edf2f7; color: var(--text-main);">Cancelar</button>
